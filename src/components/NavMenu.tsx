...
export default function NavMenu(props: Props) {
  ...
  return (
    <nav class='w-full flex flex-col items-center sm:ml-2 sm:flex-row sm:justify-end sm:py-0 sm:space-x-4'>
      ...
      >
        <NavMenuItem active={props.activeNav === 'posts'} route='posts'>文章</NavMenuItem>
        <NavMenuItem active={props.activeNav === 'tags'} route='tags'>标签</NavMenuItem>
        <NavMenuItem active={props.activeNav === 'about'} route='about'>关于</NavMenuItem>
        <NavMenuItem
          active={props.activeNav === 'search'}
          route='search'
          spanSingle
          aProps={{ tabindex: '0', 'aria-label': 'search', title: 'Search', 'aria-disabled': 'false' }}
        >
          <i class='i-custom-search mx-auto block scale-125 text-2xl sm:scale-100' classList={{ 'fill-accent': props.activeNav === 'search' }} />
        </NavMenuItem>
